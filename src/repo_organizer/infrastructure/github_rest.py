...
from repo_organizer.domain.source_control.models import Repository, LanguageBreakdown, Commit, Contributor
...
class GitHubRestAdapter(SourceControlPort):
    ...
    def get_repositories(self, limit: int = 100) -> Sequence[Repository]:
        ...
        """
        if self.logger:
            self.logger.log(f"Fetching up to {limit} repositories for {self.github_username}")
        ...
        while len(repos) < limit:
            ...
            if response.status_code != 200:
                if self.logger:
                    self.logger.log(f"Error fetching repos: {response.status_code} - {response.text}", "error")
                break
    ...
    def get_repository_languages(self, repo_name: str) -> Dict[str, float]:
        ...
        if response.status_code != 200:
            if self.logger:
                self.logger.log(f"Error fetching languages: {response.status_code}", "error")
            return {}
    ...
    def get_repository_readme(self, repo_name: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                if self.logger:
                    self.logger.log(f"Error fetching README: {response.status_code}", "warning")
                return ""
    ...
    def list_repositories(
        ...
        if owner != self.github_username:
            if self.logger:
                self.logger.log(f"Owner mismatch: {owner} != {self.github_username}", "warning")
    ...
    def contributors(self, repo: Repository) -> Sequence[Contributor]:
        ...
        try:
            ...
            if self.logger:
                self.logger.log(f"Error fetching contributors for {repo.name}: {e}", "error")
            return []